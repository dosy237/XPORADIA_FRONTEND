    ...
    <View className="gap-3 pt-1">
      ...
      <View className="gap-2">
        ...
        <View className="flex-row gap-2">
          <Chip
            label="Masculin"
            variant={sex === "M" ? "navy" : "neutral"}
            onPress={() => setSex("M")}
          />
          <Chip
            label="Féminin"
            variant={sex === "F" ? "navy" : "neutral"}
            onPress={() => setSex("F")}
          />
        </View>
      </View>
...
export default function ParentProfileScreen() {
  ...
  const addChildMutation = useMutation({
    ...
    onSuccess: (child) => {
      queryClient.setQueryData<parentApi.ParentProfile | undefined>(
        ["parent-profile"],
        (prev) =>
  ...
  const deleteChildMutation = useMutation({
    ...
    onSuccess: (_data, id) => {
      queryClient.setQueryData<parentApi.ParentProfile | undefined>(
        ...
        (prev) =>
          prev
            ? { ...prev, children: prev.children.filter((c) => c.id !== id) }
            : prev,
      );
      setDeletingId(null);
    },
  ...
  const updateChildMutation = useMutation({
    mutationFn: ({
      ...
      id: number;
      data: Partial<parentApi.ChildInput>;
        ...
        <View className="gap-3">
          ...
          {profile.children.map((child) => (
            <ChildCard
              ...
              onUpdate={(data) =>
                updateChildMutation.mutate({ id: child.id, data })
              }
            />
          ))}
