      ...
      <View className="items-center pt-10 pb-5">
        ...
        <View className="mt-2 flex-row gap-2">
          ...
          {certStatus?.current_level && (
            <Chip
              label={`${LEVEL_LABELS[certStatus.current_level]} · ${certStatus.total_points} pts`}
              variant="orange"
            />
          )}
        </View>
            ...
            <View className="bg-white rounded-3xl p-6 shadow-soft gap-5">
              <View className="flex-row gap-3">
                ...
                <StatBox
                  ...
                  value={
                    certStatus?.current_level
                      ? LEVEL_LABELS[certStatus.current_level]
                      : "Aucun"
                  }
                />
                <StatBox
              ...
              {profile.bio ? (
                <View className="gap-1">
                  ...
                  <Text className="text-sm text-xporadia-text-primary leading-5">
                    {profile.bio}
                  </Text>
                </View>
              ) : null}
              ...
              <Chip
                label={
                  profile.available_for_employment
                    ? "Ouvert au marché de l'emploi"
                    : "Non disponible actuellement"
                }
                variant={
                  profile.available_for_employment ? "navy-subtle" : "neutral"
                }
              />
            </View>
          ...
          <View className="bg-white rounded-3xl p-5 gap-4 shadow-soft">
            ...
            {!profile.is_documents_validated && (
              <Text className="text-xs text-xporadia-text-secondary -mt-2">
                Cette disponibilité s&apos;activera une fois votre compte
                accrédité par Xporadia.
              </Text>
            )}
            {mutation.isError && (
              <Text className="text-xs text-xporadia-red">
                La mise à jour a échoué. Vérifiez que votre compte est
                accrédité.
              </Text>
            )}
