...
function TeacherCard({ teacher }: { teacher: TeacherDirectoryCard }) {
  return (
    <Pressable
      onPress={() => router.push(`/(app)/teacher/directory/${teacher.id}`)}
      ...
      <View className="flex-1 gap-1">
        ...
        <View className="flex-row gap-1.5 mt-1">
          <Chip
            ...
            variant="navy-subtle"
          />
          {teacher.available_for_tutoring && <Chip label="Cours particuliers" variant="neutral" />}
        </View>
      </View>
    </Pressable>
  );
...
export default function TeacherDirectoryScreen() {
  ...
  const { data: teachers, isLoading } = useQuery({
    queryKey: ["teacher-directory", subject],
