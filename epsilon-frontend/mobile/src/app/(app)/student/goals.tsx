  ...
  return (
    <View
      className={`h-5 w-5 rounded-full border-2 items-center justify-center ${
        checked
          ? "bg-xporadia-green border-xporadia-green"
          : "border-xporadia-border"
      }`}
    >
...
export default function GoalsScreen() {
  ...
  const { data: goal } = useQuery({
    queryKey: ["life-goal"],
    queryFn: studentLifeApi.fetchLifeGoal,
  });
  const { data: bucketList } = useQuery({
    queryKey: ["bucket-list"],
    queryFn: studentLifeApi.fetchBucketList,
  });

  const saveGoalMutation = useMutation({
    mutationFn: (description: string) =>
      studentLifeApi.updateLifeGoal({ description }),
    onSuccess: () => {
  ...
  const addItemMutation = useMutation({
    mutationFn: () =>
      studentLifeApi.createBucketListItem({ title: newItemTitle.trim() }),
    onSuccess: () => {
      setNewItemTitle("");
    ...
    >
      <View className="gap-1">
        ...
        <Text className="text-sm text-xporadia-text-secondary">
          Ce que vous voulez faire de votre vie, et ce que vous voulez avoir
          accompli.
        </Text>
      </View>
      ...
      <Card className="gap-3">
        ...
        <TextInput
          ...
          onContentSizeChange={(e) =>
            setGoalInputHeight(Math.max(60, e.nativeEvent.contentSize.height))
          }
        ...
        <View className="gap-2">
          {(bucketList ?? []).map((item) => (
            <Pressable
              key={item.id}
              onPress={() =>
                toggleMutation.mutate({ id: item.id, is_done: !item.is_done })
              }
            ...
            >
              ...
              <Text
                className={`flex-1 text-sm ${
                  item.is_done
                    ? "text-xporadia-text-secondary line-through"
                    : "text-xporadia-text-primary font-medium"
                }`}
              >
                {item.title}
              </Text>
