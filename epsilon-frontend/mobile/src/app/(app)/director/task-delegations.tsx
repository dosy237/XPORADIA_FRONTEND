...
import { ClockIcon, GraduationCapIcon, TrashIcon, UserPlusIcon } from "@/components/ui/Icon";
import type { IconProps } from "@/components/ui/Icon";
import { Input } from "@/components/ui/Input";
import { Colors } from "@/constants/theme";
import * as academicsApi from "@/services/academics";
import type { DelegatedTask, TaskDelegationEntry } from "@/services/academics";
...
const TASK_ICONS: Record<DelegatedTask, ComponentType<IconProps>> = {
  timetable: ClockIcon,
  join_requests: GraduationCapIcon,
};

const ALL_TASKS = Object.keys(academicsApi.TASK_LABELS) as DelegatedTask[];

function TaskDelegationCard({
  task,
  delegates,
  isLoading,
}: {
  task: DelegatedTask;
  delegates: TaskDelegationEntry[];
  isLoading: boolean;
}) {
  const queryClient = useQueryClient();
  const [email, setEmail] = useState("");
  const [error, setError] = useState("");
  const TaskIcon = TASK_ICONS[task];

  const addMutation = useMutation({
    mutationFn: () => academicsApi.addTaskDelegation(task, email.trim().toLowerCase()),
    onSuccess: () => {
      setEmail("");
      setError("");
  ...
  const removeMutation = useMutation({
    mutationFn: (teacherEmail: string) => academicsApi.removeTaskDelegation(task, teacherEmail),
  ...
  return (
    <View className="bg-white rounded-2xl p-4 shadow-soft gap-3">
      <View className="flex-row items-center gap-2">
        <TaskIcon size={16} color={Colors.navy} />
        <Text className="text-sm font-bold text-xporadia-navy">{academicsApi.TASK_LABELS[task]}</Text>
      </View>
      <Text className="text-xs text-xporadia-text-secondary leading-5">{TASK_DESCRIPTIONS[task]}</Text>
    ...
    <ScrollView className="flex-1 bg-xporadia-bg" contentContainerClassName="p-6 gap-5 pb-12">
      ...
      {ALL_TASKS.map((task) => (
        <TaskDelegationCard
          key={task}
          task={task}
          delegates={(delegations ?? []).filter((d) => d.task === task)}
          isLoading={isLoading}
        />
      ))}
